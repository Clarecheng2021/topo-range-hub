...
`range-net` 设置为 `internal: true`，PLC 模拟器不连接外部网络且不发布端口。HMI 同时连接到仅用于本机端口转发的 `hmi-edge` 网络；其 Web 入口仍只绑定到 `127.0.0.1`，不会暴露到局域网。

HMI 是静态只读页面，不含任何外部资源。若部署环境要求连静态 Web 服务也不得存在出站路径，应在宿主机防火墙或网络策略中禁止 `hmi` 容器的出站访问。
